...
class PrHrSet(Dataset):
    def __init__(self, samples_dir, filenames=None):
        ...
        if filenames is not None:
            files = [
                ...
                if any(i for j in filenames if j in i)
            ]
        else:
    ...
    def __getitem__(self, i):
        lr_i, lr_sr = torchaudio.load(
            os.path.join(self.samples_dir, self.lr_filenames[i])
        )
        hr_i, hr_sr = torchaudio.load(
            os.path.join(self.samples_dir, self.hr_filenames[i])
        )
        pr_i, pr_sr = torchaudio.load(
            os.path.join(self.samples_dir, self.pr_filenames[i])
        )
