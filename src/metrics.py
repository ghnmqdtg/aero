...
def get_visqol(ref_sig, out_sig, filename, sr, speech_mode, visqol_path):
    ...
    try:
        tfm.build_file(
            input_array=ref_sig, sample_rate_in=sr, output_filepath=reference_abs_path
        )
        tfm.build_file(
            input_array=out_sig, sample_rate_in=sr, output_filepath=estimation_abs_path
        )
        while not os.path.exists(reference_abs_path) and not os.path.exists(
            estimation_abs_path
        ):
        ...
        if not os.path.isfile(reference_abs_path):
            raise FileNotFoundError(
                errno.ENOENT, os.strerror(errno.ENOENT), reference_abs_path
            )
        if not os.path.isfile(estimation_abs_path):
            raise FileNotFoundError(
                errno.ENOENT, os.strerror(errno.ENOENT), estimation_abs_path
            )
        ...
        visqol = subprocess.run(
            visqol_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
