...
class Solver(object):
    def __init__(self, data, models, optimizers, args):
        ...
        self.adversarial_mode = (
            "adversarial" in args.experiment and args.experiment.adversarial
        )
        ...
        if (
            "discriminator_model" in self.args.experiment
            and self.args.experiment.discriminator_model == "hifi"
        ):
    ...
    def _load(self, package, load_best=False):
        if load_best:
            for name, model_package in package[SERIALIZE_KEY_BEST_STATES][
                SERIALIZE_KEY_MODELS
            ].items():
    ...
    def train(self):
        ...
        for epoch in range(len(self.history), self.epochs):
            ...
            logger_msg = (
                f"Train Summary | End of Epoch {epoch + 1} | Time {time.time() - start:.2f}s | "
            ...
            if (
                self.cross_valid
                and (
                    (epoch + 1) % self.cross_valid_every == 0
                ...
                with torch.no_grad():
                    ...
                    if self.args.valid_equals_test:
                        ...
                        valid_losses, enhanced_filenames = (
                            self._get_valid_losses_on_test_data(
                ...
                logger_msg = (
                    ...
                    + " | ".join(
                        [f"{k} Valid Loss {v:.5f}" for k, v in valid_losses.items()]
                    )
                )
                logger.info(bold(logger_msg))
                valid_losses = {
                    "valid_" + k + "_loss": v for k, v in valid_losses.items()
                }

                best_loss = min(
                    pull_metric(self.history, "valid_evaluation_loss")
                    + [evaluation_loss]
                )
                # Save the best model
            ...
            if (
                ...
                with swap_state(self.model, best_state):
                    ...
                    if evaluated_on_test_data:
                        logger.info(
                            "Samples already evaluated in cross validation, calculating metrics."
                        )
                        enhanced_dataset = PrHrSet(
                            self.args.samples_dir, enhanced_filenames
                        )
                        ...
                        lsd, visqol = evaluate_on_saved_data(
                            self.args, enhanced_dataloader, epoch
                        )
                        ...
                        enhanced_dataloader = DataLoader(
                            enhanced_dataset, batch_size=1, shuffle=False
                        )
                        lsd, visqol = evaluate_on_saved_data(
                            self.args, enhanced_dataloader, epoch
                        )
    ...
    def _run_one_epoch(self, epoch, cross_valid=False):
        ...
        return_spec = (
            "return_spec" in self.args.experiment and self.args.experiment.return_spec
        )
        ...
        for i, data in enumerate(logprog):
            ...
            logprog.update(memory_usage=f"{torch.cuda.memory_allocated() / 1e9:.2f} GB")
    ...
    def _get_valid_losses_on_test_data(self, epoch, enhance):
        ...
        for i, data in enumerate(logprog):
            ...
            if self.args.experiment.model == "aero":
                ...
                pr_time, pr_spec, lr_spec = self.dmodel(
                    lr, return_spec=True, return_lr_spec=True
                )
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            if self.adversarial_mode:
                if "msd_melgan" in self.args.experiment.discriminator_models:
                    generator_losses, discriminator_loss = (
                        self._get_melgan_adversarial_loss(pr_time, hr_time)
                    )
                    if not self.args.experiment.only_features_loss:
                        losses["generator"].update(
                            {"adversarial_melgan": generator_losses["adversarial"]}
                        )
                    if not self.args.experiment.only_adversarial_loss:
                        losses["generator"].update(
                            {"features_melgan": generator_losses["features"]}
                        )
                    losses["discriminator"].update({"msd_melgan": discriminator_loss})
                if "msd_hifi" in self.args.experiment.discriminator_models:
                    generator_losses, discriminator_loss = (
                        self._get_msd_adversarial_loss(pr_time, hr_time)
                    )
                    if not self.args.experiment.only_features_loss:
                        losses["generator"].update(
                            {"adversarial_msd": generator_losses["adversarial"]}
                        )
                    if not self.args.experiment.only_adversarial_loss:
                        losses["generator"].update(
                            {"features_msd": generator_losses["features"]}
                        )
                    losses["discriminator"].update({"msd": discriminator_loss})
                if "mpd" in self.args.experiment.discriminator_models:
                    generator_losses, discriminator_loss = (
                        self._get_mpd_adversarial_loss(pr_time, hr_time)
                    )
                    if not self.args.experiment.only_features_loss:
                        losses["generator"].update(
                            {"adversarial_mpd": generator_losses["adversarial"]}
                        )
                    if not self.args.experiment.only_adversarial_loss:
                        losses["generator"].update(
                            {"features_mpd": generator_losses["features"]}
                        )
                    losses["discriminator"].update({"mpd": discriminator_loss})
                if "hifi" in self.args.experiment.discriminator_models:
                    generator_loss, discriminator_loss = (
                        self._get_hifi_adversarial_loss(pr_time, hr_time)
                    )
    ...
    def _get_melgan_adversarial_loss(self, pr, hr):
        ...
        total_loss_discriminator = self._get_melgan_discriminator_loss(
            discriminator_fake_detached, discriminator_real
        )
        generator_losses = self._get_melgan_generator_loss(
    ...
    def _get_melgan_generator_loss(self, discriminator_fake, discriminator_real):
        features_loss = 0
        features_weights = 4.0 / (
            self.args.experiment.melgan_discriminator.n_layers + 1
        )
        ...
        for i in range(self.args.experiment.melgan_discriminator.num_D):
            for j in range(len(discriminator_fake[i]) - 1):
                features_loss += weights * F.l1_loss(
                    discriminator_fake[i][j], discriminator_real[i][j].detach()
                )
        ...
        if (
            "only_adversarial_loss" in self.args.experiment
            and self.args.experiment.only_adversarial_loss
        ):
            return {"adversarial": adversarial_loss}

        if (
            "only_features_loss" in self.args.experiment
            and self.args.experiment.only_features_loss
        ):
            return {
    ...
    def _get_msd_adversarial_loss(self, pr, hr):
        ...
        if (
            ...
            return {
                "features": self.args.experiment.features_loss_lambda * g_feat_loss
            }, d_loss
    ...
    def _optimize_adversarial(self, discriminator_losses):
        ...
        self.scaler_D.step(disc_optimizer)
