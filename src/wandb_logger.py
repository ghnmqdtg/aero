...
def log_data_to_wandb(
    ...
    enhanced_spectrogram_wandb_image = wandb.Image(
        convert_spectrogram_to_heatmap(enhanced_spectrogram), caption="PR"
    )
    enhanced_wandb_audio = wandb.Audio(
        pr_signal.squeeze().numpy(), sample_rate=hr_sr, caption="PR"
    )

    wandb_dict = {
        f"test samples/{filename}/lsd": lsd,
        f"test samples/{filename}/lsd_hf": lsd_hf,
        f"test samples/{filename}/lsd_lf": lsd_lf,
    ...
    if pr_spec is not None and hr_spec is not None and lr_spec is not None:
        ...
        wandb_dict.update(
            {f"test samples/{filename}/pr_spec": enhanced_pr_spectrogram_wandb_image}
        )
        ...
        if epoch <= 10:
            ...
            enhanced_hr_spectrogram_wandb_image = wandb.Image(
                hr_spec, caption="HR spec"
            )
            wandb_dict.update(
                {
                    f"test samples/{filename}/hr_spec": enhanced_hr_spectrogram_wandb_image
                }
            )
            ...
            enhanced_lr_spectrogram_wandb_image = wandb.Image(
                lr_spec, caption="LR spec"
            )
            wandb_dict.update(
                {
                    f"test samples/{filename}/lr_spec": enhanced_lr_spectrogram_wandb_image
                }
            )

    if epoch <= 10:
        hr_name = f"{filename}_hr"
        hr_enhanced_spectrogram = (
            spectrogram_transform(hr_signal).log2()[0, :, :].numpy()
        )
        hr_enhanced_spectrogram_wandb_image = wandb.Image(
            convert_spectrogram_to_heatmap(hr_enhanced_spectrogram), caption="HR"
        )
        hr_enhanced_wandb_audio = wandb.Audio(
            hr_signal.squeeze().numpy(), sample_rate=hr_sr, caption="HR"
        )
        wandb_dict.update(
            {
        ...
        lr_enhanced_spectrogram = (
            spectrogram_transform(lr_signal).log2()[0, :, :].numpy()
        )
        lr_enhanced_spectrogram_wandb_image = wandb.Image(
            convert_spectrogram_to_heatmap(lr_enhanced_spectrogram), caption="LR"
        )
        lr_enhanced_wandb_audio = wandb.Audio(
            lr_signal.squeeze().numpy(), sample_rate=lr_sr, caption="LR"
        )
        wandb_dict.update(
            {
...
def create_wandb_table(args, data_loader, epoch):
    ...
    for i, data in enumerate(data_loader):
        if (
            args.wandb.n_files_to_log_to_table
            and i >= args.wandb.n_files_to_log_to_table
        ):
            break
        lr, hr, pr, filename = data
        filename = filename[0]
        lsd, lsd_hf, lsd_lf, visqol = run_metrics(hr, pr, args, filename)
        add_data_to_wandb_table(
            (hr, lr, pr), (lsd, lsd_hf, lsd_lf, visqol), filename, args, wandb_table
        )
...
def add_data_to_wandb_table(signals, metrics, filename, args, wandb_table):
    ...
    lr_spectrogram = (
        (SPECTOGRAM_EPSILON + spectrogram_transform(lr_upsampled))
        .log2()[0, :, :]
        .numpy()
    )
    ...
    lsd, lsd_hf, lsd_lf, visqol = metrics

    hr_sr = args.experiment.hr_sr
    lr_sr = args.experiment.lr_sr

    hr_wandb_audio = wandb.Audio(
        hr.squeeze().numpy(), sample_rate=hr_sr, caption=filename + "_hr"
    )
    lr_wandb_audio = wandb.Audio(
        lr.squeeze().numpy(), sample_rate=lr_sr, caption=filename + "_lr"
    )
    pr_wandb_audio = wandb.Audio(
        pr.squeeze().numpy(), sample_rate=hr_sr, caption=filename + "_pr"
    )
